...
# Mount routers
from routers.reports import router as reports_router, _pending_uploads, _batches  # noqa: E402
...
async def batch_pipeline_ws(websocket: WebSocket, batch_id: str):
    """
    Stream concurrent pipeline progress for all reports in a batch.

    Opens after POST /reports/batch-upload returns {batch_id}.
    Runs all report pipelines concurrently via asyncio.gather, emitting per-report
    progress messages and overall batch progress. Closes when all reports are done.
    """
    await websocket.accept()
    ...
    if not report_ids:
        await send({"error": "No pending batch found for this batch_id", "batch_done": True})
        await websocket.close()
        return
    ...
    # Build per-report tasks
    async def process_one(report_id: str):
        nonlocal completed_count

        entry = _pending_uploads.pop(report_id, None)
        if entry is None:
            await send({"report_id": report_id, "status": "error",
                        "message": "No pending upload found", "done": True})
            completed_count += 1
            await send({"batch_progress": round(completed_count / total * 100),
                        "completed": completed_count, "total": total})
            return

        file_bytes, filename = entry

        async def progress_callback(stage_num: int, stage_name: str, status: str,
        ...
        try:
            ...
            logger.error(f"[WS batch] Unhandled error for report {report_id}: {exc}")
            await send({"report_id": report_id, "filename": filename,
                        "stage": 0, "progress": 0, "status": "error",
                        "message": str(exc), "done": True})
            completed_count += 1
            await send({"batch_progress": round(completed_count / total * 100),
                        "completed": completed_count, "total": total})
        ...
        async for db in get_db():
            try:
                ...
                if report_row:
                    ...
                    if patient_row:
                        patient_row.name = patient_row.name or patient_info.get("name")
                        patient_row.age = patient_row.age or patient_info.get("age")
                        patient_row.gender = patient_row.gender or patient_info.get("gender")

                    report_row.document_type = result.get("document_type", "unknown")
                    report_row.raw_text = result.get("raw_text", "")
                    report_row.status = final_status
                    report_row.analysis_result = result.get("clinical_report")
                ...
                logger.error(f"[WS batch] DB save failed for {report_id}: {db_exc}")
                await db.rollback()
            break
        ...
        completed_count += 1
        await send({
            "batch_progress": round(completed_count / total * 100),
    ...
    await _asyncio.gather(*[process_one(rid) for rid in report_ids])

    await send({"batch_done": True, "report_ids": report_ids})
    try:
        await websocket.close()
    except Exception:
        pass
