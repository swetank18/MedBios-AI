...
function AppContent() {
  ...
  const NAV_LINKS = [
    { to: '/', label: t('dashboard'), end: true, icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { to: '/upload', label: t('upload_report'), icon: 'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5' },
    { to: '/drug-interactions', label: t('drug_checker'), icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z' },
  ];
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
          ...
          <NavLink to="/" className="flex items-center gap-2.5 font-bold text-lg group">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[0.55rem] text-[#4a9175] font-medium tracking-widest uppercase">{t('clinical_intelligence')}</span>
            </div>
          </NavLink>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-bg-secondary transition text-text-secondary"
                title={t('language')}
              >
                {/* Globe icon */}
                <svg className="w-4 h-4 text-accent-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253M3 12a8.959 8.959 0 011.716-5.336" />
                </svg>
                <span className="hidden sm:block text-xs font-medium">{currentLang.label}</span>
                <svg className={`w-3 h-3 text-text-muted transition-transform ${langMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
              </button>

              {langMenuOpen && (
                <div className="absolute right-0 top-full mt-1 w-44 rounded-xl border border-border-subtle bg-bg-card/95 backdrop-blur-xl shadow-xl shadow-black/20 py-1 z-50 fade-in">
                  {LANGUAGES.map(l => (
                    <button
                      key={l.code}
